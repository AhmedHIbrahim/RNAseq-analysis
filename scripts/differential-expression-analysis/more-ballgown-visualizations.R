...
pdf(file="Final_Part3_Supplementary_R_output.pdf")

# set working dir
working_dir = "D:\__Ph.D\1st\BINF6002 -  BIOLOGICAL SEQUENCE ANALYSIS\scripts\rnaseq-de"
setwd(working_dir)
...
gene_expression[1:3, c("FPKM.Ad-Rbm10_rep1","FPKM.Ad-Gfb_rep1")]
...
#First determine the rows in the data.frame that match 'TST', aka. ENSG00000000005, then display only those rows of the data.frame

i = row.names(gene_expression) == "ENSG00000182872"
gene_expression[i,]

#What if we want to view values for a list of genes of interest all at once? 
# genes_of_interest = c("RBM10", "RBM43", "RBM5")

gene_id_name = data.frame(id=ballgown::geneIDs(bg),name=ballgown::geneNames(bg))
gene_id_name[which(gene_id_name$name == "RBM10"),]$id
gene_id_name[which(gene_id_name$name == "RBM43"),]$id
gene_id_name[which(gene_id_name$name == "RBM5"),]$id

genes_of_interest = c("ENSG00000182872", "ENSG00000184898", "ENSG00000003756")

genes_index=which(ballgown::geneNames(bg) %in% genes_of_interest)
...
c_max = max(counts)
hist(counts, breaks=100, col="bisque4", xlab="Transcripts per gene", main="Distribution of transcript count per gene")
...
min(gene_expression[,"FPKM.Ad-Rbm10_rep1"])
max(gene_expression[,"FPKM.Ad-Gfb_rep1"])
...
x = gene_expression[,"FPKM.Ad-Rbm10_rep1"]
y = gene_expression[,"FPKM.Ad-Rbm10_rep1"]
plot(x=log2(x+min_nonzero), y=log2(y+min_nonzero), pch=16, col="blue", cex=0.25, xlab="FPKM (Ad-Rbm10, Replicate 1)", ylab="FPKM (Ad-Rbm10, Replicate 2)", main="Comparison of expression values for a pair of replicates")
...
smoothScatter(x=log2(x+min_nonzero), y=log2(y+min_nonzero), xlab="FPKM (Ad-Rbm10, Replicate 1)", ylab="FPKM (Ad-Rbm10, Replicate 2)", main="Comparison of expression values for a pair of replicates", colramp=colors, nbin=200)
...
plotCor("FPKM.Ad-Rbm10_rep1", "FPKM.Ad-Gfb_rep1", "Ad-Rbm10_rep1 vs Ad-Gfb_rep1", "tomato2")
plotCor("FPKM.Ad-Rbm10_rep2", "FPKM.Ad-Gfb_rep2", "Ad-Rbm10_rep2 vs Ad-Gfb_rep2", "royalblue2")
...
par(mfrow=c(1,2))
plotCor2("FPKM.Ad-Rbm10_rep1", "FPKM.Ad-Gfb_rep1", "Ad-Rbm10_rep1 vs Ad-Gfb_rep1", "tomato2")
plotCor2("FPKM.Ad-Rbm10_rep2", "FPKM.Ad-Gfb_rep2", "Ad-Rbm10_rep2 vs Ad-Gfb_rep2", "royalblue2")
...
hist(results_genes[sig,"de"], breaks=50, col="seagreen", xlab="log2(Fold change) Ad-Rbm10 vs Ad-Gfb", main="Distribution of differential expression values")
...
#### Plot #10 - Display the grand expression values from Ad-Rbm10 and Ad-Gfb and mark those that are significantly differentially expressed
gene_expression[,"Ad-Rbm10"]=apply(gene_expression[,c(1:3)], 1, mean)
gene_expression[,"Ad-Gfb"]=apply(gene_expression[,c(4:6)], 1, mean)

x=log2(gene_expression[,"Ad-Rbm10"]+min_nonzero)
y=log2(gene_expression[,"Ad-Gfb"]+min_nonzero)
plot(x=x, y=y, pch=16, cex=0.25, xlab="Ad-Rbm10 FPKM (log2)", ylab="Ad-Gfb FPKM (log2)", main="Ad-Rbm10 vs Ad-Gfb FPKMs")
abline(a=0, b=1)
xsig=x[sig]
ysig=y[sig]
